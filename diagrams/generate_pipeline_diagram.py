#!/usr/bin/env python3
"""
Diagram Generation Script for Scaffold AI Data Pipeline
Generates a Mermaid flow chart and converts it to an image file.
"""
...
def generate_image_with_mermaid_cli(mermaid_code, output_path="pipeline_diagram.png"):
    """Generate image using mermaid-cli if available."""
    try:
        ...
        result = subprocess.run([
            ...
            '-o', output_path,
            '-b', 'transparent'
        ], input=mermaid_code.encode(), capture_output=True, text=True)
        
        if result.returncode == 0:
            print(f"✓ Image generated using mermaid-cli: {output_path}")
            return True
        else:
...
def generate_image_with_puppeteer(mermaid_code, output_path="pipeline_diagram.png"):
    """Generate image using Puppeteer if available."""
    try:
...
{mermaid_code}
    ...
    <script>
        ...
        # Try to use Puppeteer or similar
        try:
            import puppeteer
            # This would require puppeteer-python to be installed
            print("Puppeteer method not fully implemented")
            return False
        except ImportError:
        ...
        print(f"Puppeteer method failed: {e}")
        return False

def generate_image_with_playwright(mermaid_code, output_path="pipeline_diagram.png"):
...
def save_mermaid_file(mermaid_code, output_path="pipeline_diagram.mmd"):
...
def main():
    ...
    print("Generating Scaffold AI Data Pipeline Diagram...")
    ...
    if not image_generated:
        image_generated = generate_image_with_mermaid_cli(mermaid_code)
    
    # Method 2: Try Playwright
    if not image_generated:
        image_generated = generate_image_with_playwright(mermaid_code)
    
    # Method 3: Try Puppeteer
    if not image_generated:
        image_generated = generate_image_with_puppeteer(mermaid_code)
    ...
    if not image_generated:
        ...
        print("2. Run: mmdc -i pipeline_diagram.mmd -o pipeline_diagram.png")
