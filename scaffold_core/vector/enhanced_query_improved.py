...
# Constants
TOP_K_INITIAL = 30
TOP_K_FINAL = 3  # Use fewer final sources to reduce reliance on retrieval
MIN_CROSS_SCORE = 0.0  # Stricter: only keep sources with positive cross-encoder score
MIN_CONTEXTUAL_SCORE = 2  # Stricter: require at least 2 keyword overlaps
...
Your role is to provide a concise, well-structured answer primarily from your internal knowledge. Use external sources only to support, not dominate, your response.

Please provide an answer that:
1. Directly addresses the question
2. Keeps external citations minimal (max 3 brief mentions)
3. Emphasizes your own knowledge and synthesis of best practices
4. Suggests practical, implementable steps"""
...
class ImprovedEnhancedQuerySystem:
    ...
    def extract_keywords(self, text: str) -> List[str]:
        ...
        # Light normalization: remove punctuation, lowercase, collapse whitespace
    ...
    def _text_normalize(self, text: str) -> str:
        """Perform light text normalization: lowercase, remove punctuation, collapse whitespace."""
        if not text:
            return ""
        # Lowercase
        text = text.lower()
    ...
    def cross_encoder_rerank(self, query: str, candidates: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            # Add cross-encoder scores to candidates and normalize text for context checks
            for candidate, score in zip(candidates, cross_scores):
                candidate["cross_score"] = float(score)
                # Normalize candidate text to ensure consistent keyword extraction
                candidate_text = candidate.get("text", "")
                candidate["_normalized_text"] = self._text_normalize(candidate_text)

                # Filter out candidates below the minimum cross score
    ...
    def contextual_filtering(self, query: str, candidates: List[Dict]) -> List[Dict]:
        ...
        try:
            # Normalize query first for consistent keyword extraction
            normalized_query = self._text_normalize(query)
            query_keywords = set(self.extract_keywords(normalized_query))
            if not query_keywords:
                return candidates
            
            for candidate in candidates:
                # Prefer normalized text if available (from cross-encoder step)
                candidate_text = candidate.get('_normalized_text') or candidate.get('text', '')
                text_keywords = set(self.extract_keywords(candidate_text))
                shared_keywords = query_keywords.intersection(text_keywords)
                candidate['contextual_score'] = len(shared_keywords)
                # Filter out candidates that don't meet the stricter contextual score
            ...
            # Sort by a combination: prioritize cross_score then contextual_score
            candidates.sort(key=lambda x: (x.get('cross_score', 0), x.get('contextual_score', 0)), reverse=True)
...
<|assistant|>"""
    ...
    def _is_response_garbled(self, response: str) -> bool:
        """Garbled detection disabled: always return False."""
        return False
...
{query}{context} [/INST]"""
    ...
    def query(self, query: str, session_id: Optional[str] = None) -> Dict[str, Any]:
        """Process a query and return relevant results with improved prompt engineering. Logs all context, prompts, outputs, and timing."""
        import time
        ...
        try:
            ...
            if session_id:
                ...
                logger.info(f"[LOG] Conversation context ({len(conversation_context.split())} tokens):\n{conversation_context}")

            # Log query processing
            logger.info(f"[LOG] Processing query: {query}" + (f" (session: {session_id})" if session_id else ""))

            # Get initial candidates using hybrid search
            candidates = self.hybrid_search(query)
            logger.info(f"[LOG] Found {len(candidates)} initial candidates")

            if not candidates:
                logger.warning("[LOG] No candidates found for query")
                return {
            ...
            logger.info(f"[LOG] Reranked to {len(reranked_candidates)} candidates")

            # Apply contextual filtering
            filtered_candidates = self.contextual_filtering(query, reranked_candidates)
            logger.info(f"[LOG] Filtered to {len(filtered_candidates)} candidates")

            # Use all filtered candidates instead of limiting to top 5
            final_candidates = filtered_candidates
            logger.info(f"[LOG] Using all {len(final_candidates)} filtered candidates")

            # Generate improved prompt
            improved_prompt = self.generate_improved_prompt(query, final_candidates, conversation_context)
            logger.info(f"[LOG] Improved prompt passed to LLM:\n{improved_prompt}")

            # Get temperature from config
            from scaffold_core.config import LLM_TEMPERATURE, LLM_DEVICE
            temperature = LLM_TEMPERATURE
            logger.info(f"[LOG] LLM device: {LLM_DEVICE}")
            ...
            try:
                ...
                logger.info(f"[LOG] LLM response (first attempt):\n{llm_response}")

                # Validate response quality
                if self._is_response_garbled(llm_response):
                    logger.warning("[LOG] Detected garbled response, retrying with reduced context")
                    raise ValueError("Garbled response detected")

            except Exception as e:
                logger.error(f"[LOG] Error in LLM response generation: {e}")

                # Try with reduced context
                try:
                    logger.warning("[LOG] Retrying with minimal context")
                    minimal_prompt = self._generate_minimal_prompt(query, final_candidates[:1])
                    logger.info(f"[LOG] Minimal prompt:\n{minimal_prompt}")
                    llm_response = get_llm().generate_response_with_continuation(minimal_prompt, temperature=temperature)
                    logger.info(f"[LOG] LLM response (minimal context):\n{llm_response}")

                    # Validate response again
                    if self._is_response_garbled(llm_response):
                        logger.error("[LOG] Still getting garbled response, using fallback")
                        llm_response = self._generate_fallback_response(query)

                except Exception as e2:
                    logger.error(f"[LOG] Second attempt failed: {e2}")
                    llm_response = self._generate_fallback_response(query)
            ...
            if len(llm_response) > 8000:  # Increased from 2000 to allow longer responses
                ...
                logger.warning("[LOG] Response truncated due to length limit (8000 chars)")
            ...
            logger.info(f"[LOG] Query completed in {elapsed:.2f} seconds")
            ...
            logger.error(f"[LOG] Error processing query: {e}")
            return {
