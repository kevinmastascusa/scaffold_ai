...
class ImprovedEnhancedQuerySystem:
    ...
    def __init__(self):
        ...
        self.auto_clear_on_topic_shift = str(
            os.getenv("SC_AUTO_CLEAR_ON_TOPIC_SHIFT", "")
        ).lower() in ("1", "true", "yes")
        try:
            self.topic_shift_threshold = float(
                os.getenv("SC_TOPIC_SHIFT_THRESHOLD", "0.2")
            )  # 0..1 overlap
        except Exception:
    ...
    def add_to_memory(self, session_id: str, message: Dict[str, Any]):
        ...
        if len(self.conversation_memory[session_id]) > MAX_MEMORY_MESSAGES:
            self.conversation_memory[session_id] = (
                self.conversation_memory[session_id][-MAX_MEMORY_MESSAGES:]
            )

        logger.debug(
            f"Added message to memory for session {session_id}. "
            f"Total messages: {len(self.conversation_memory[session_id])}"
        )
    ...
    def query(self, query: str, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if enable_tot:
                    try:
                        breadth = int(os.getenv("SC_TOT_BREADTH", "3") or "3")
                    except Exception:
                        breadth = 3
                    try:
                        depth = int(os.getenv("SC_TOT_DEPTH", "2") or "2")
                    except Exception:
                        depth = 2

                    # Prefer ToT path when enabled
                    llm_response = get_llm().tot_generate(  # type: ignore[attr-defined]
                        improved_prompt,
                        breadth=breadth,
                        depth=depth,
